...
HRESULT CTitle::Init(void)
{
	...
	{//	アイテムの設定	位置　回転　大きさ　色　UV　種類
		CItem::Create
		(m_textState[OBJTYPE_ITEM].pos[nCount], D3DXVECTOR3(0.0f, 0.0f, 0.0f), m_textState[OBJTYPE_ITEM].size[nCount], m_textState[OBJTYPE_ITEM].col[nCount], m_textState[OBJTYPE_ITEM].uv[nCount], m_textState[OBJTYPE_ITEM].nType[nCount]);
	}
...
void CTitle::Update(void)
{
	...
	{// フェードしていない状態
		...
		{// キーボード（ENTER）を入力したら
			CFade::Create(CManager::MODE_SELECTMODE, CFade::FADE_OUT,3);
