...
void InitSelectmode(void)
{
	...
	g_Selectmode[0] = D3DXVECTOR2(340, 70);//メインゲーム
	g_Selectmode[1] = D3DXVECTOR2(340, 200);
	g_Selectmode[2] = D3DXVECTOR2(340, 330);
	g_Selectmode[4] = D3DXVECTOR2(870, 125);//SELECTMODE
	g_Selectmode[5] = D3DXVECTOR2(870, 385);//ボタンのやり方
	g_Selectmode[6] = D3DXVECTOR2(300, 600);
	...
	g_pVtxBuffSelectmode->Lock(0, 0, (void**)&pVtx, 0);
		...
		pVtx[0].pos = D3DXVECTOR3(200.0f, 470.0f, 0.0f);		 //右上
		pVtx[1].pos = D3DXVECTOR3(900.0f, 470.0f, 0.0f);		 //右下
		pVtx[2].pos = D3DXVECTOR3(200.0f, 670.0f, 0.0f);		 //左上
		pVtx[3].pos = D3DXVECTOR3(900.0f, 670.0f, 0.0f);		 //左下
		//RHW
		pVtx[0].rhw = 1.0f;
		pVtx[1].rhw = 1.0f;
		pVtx[2].rhw = 1.0f;
	...
	{
		//頂点設定
		pVtx[16].pos = D3DXVECTOR3(g_Selectmode[nCount].x - 270.0f, g_Selectmode[nCount].y - 100.0f, 0.0f);
		pVtx[17].pos = D3DXVECTOR3(g_Selectmode[nCount].x + 270.0f, g_Selectmode[nCount].y - 100.0f, 0.0f);
		pVtx[18].pos = D3DXVECTOR3(g_Selectmode[nCount].x - 270.0f, g_Selectmode[nCount].y + 100.0f, 0.0f);
		pVtx[19].pos = D3DXVECTOR3(g_Selectmode[nCount].x + 270.0f, g_Selectmode[nCount].y + 100.0f, 0.0f);
		//RHW
		pVtx[16].rhw = 1.0f;
		pVtx[17].rhw = 1.0f;
		pVtx[18].rhw = 1.0f;
	...
	//頂点設定
	pVtx[44].pos = D3DXVECTOR3(165.0f, 460.0f, 0.0f);		 //右上
	pVtx[45].pos = D3DXVECTOR3(935.0f, 460.0f, 0.0f);		 //右下
	pVtx[46].pos = D3DXVECTOR3(165.0f, 685.0f, 0.0f);		 //左上
	pVtx[47].pos = D3DXVECTOR3(935.0f, 685.0f, 0.0f);		 //左下
	//RHW
	pVtx[44].rhw = 1.0f;
	pVtx[45].rhw = 1.0f;
	pVtx[46].rhw = 1.0f;
...
void UpdateSelectmode(void)
{
	...
	if (GetKeyboardTrigger(DIK_S) == true ||  GetJoyPadTrigger(DIJS_BUTTON_11, 0) == true )
	{
		PlaySound(SOUND_LABEL_SE002);
		g_aSelectMenu = (g_aSelectMenu + 1) % 3;
		g_FadeTitle = 0;
	}
	if (GetKeyboardTrigger(DIK_W) == true || GetJoyPadTrigger(DIJS_BUTTON_10, 0) == true)
	{
		PlaySound(SOUND_LABEL_SE002);
		g_aSelectMenu = (g_aSelectMenu + 2) % 3;
		g_FadeTitle = 0;
	}
	...
	{
		//キーボード設定
		if (pFade == FADE_NONE && GetKeyboardTrigger(DIK_RETURN) == true || GetJoyPadTrigger(DIJS_BUTTON_1, 0) == true)
		{//エンターでセレクト画面へ
			switch (g_aSelectMenu)
			{
			case  0://あらすじ
				SetFade(MODO_TUTORIAL);
	...
	//頂点バッファをロックし、頂点データへのポインタ
		g_pVtxBuffSelectmode->Lock(0, 0, (void**)&pVtx, 0);
			...
			pVtx[14].pos = D3DXVECTOR3(g_Selectmode[g_aSelectMenu ].x - 270.0f, g_Selectmode[g_aSelectMenu ].y + 75.0f, 0.0f);
			pVtx[15].pos = D3DXVECTOR3(g_Selectmode[g_aSelectMenu ].x + 420.0f, g_Selectmode[g_aSelectMenu ].y + 75.0f, 0.0f);
