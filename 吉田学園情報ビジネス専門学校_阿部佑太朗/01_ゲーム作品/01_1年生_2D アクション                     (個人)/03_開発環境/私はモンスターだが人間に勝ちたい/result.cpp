...
//マクロ定義
//*****************************************************************************
#define RESULT_TEXTURENAMRE "data/TEXTURE/end001.png"  //読み込むテクスチャ
#define RESULT_TEXTURENAMRE1 "data/TEXTURE/end000.png" //読み込むテクスチャ
#define RESULT_TEXTURENAMRE2 "data/TEXTURE/Press000.png" //読み込むテクスチャ
#define MAX_RESULT (3)
...
void UpdateResult(void)
{
	...
	g_pVtxBuffRESULT->Lock(0, 0, (void**)&pVtx, 0);
	//テクスチャ座標
	pVtx[0].tex = D3DXVECTOR2(g_RS.x, 0.0f);						//右上
	pVtx[1].tex = D3DXVECTOR2(g_RS.x - 0.25f, 0.0f);					//右下
	pVtx[2].tex = D3DXVECTOR2(g_RS.x, 1.0f);						//左上
	pVtx[3].tex = D3DXVECTOR2(g_RS.x - 0.25f, 1.0f);				//左下
	//頂点バッファアンロックする
	g_pVtxBuffRESULT->Unlock();
	//エンターでモードエンターに移行
	FADE pFade;
